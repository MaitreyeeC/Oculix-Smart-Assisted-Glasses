      ...
      if (mode === MODES.SCENE) {
        ...
        if (!data || data.length === 0) {
          ...
          const txt = data
            .map(
              (d) =>
                `${d.label} ${Math.round(d.confidence * 100)} percent`
            )
            .join(", ");
          const msg = "Detected: " + txt;
          setOutput("📦 " + msg);
          speak(msg);
        }
        ...
        const data = res.data;
        if (!data || data.length === 0) {
          const msg = "No readable text found.";
          setOutput("🔤 " + msg);
          speak(msg);
        } else {
          const txt = data.map((d) => d.text).join(" . ");
          const msg = "Text reads: " + txt;
          setOutput("🔤 " + msg);
          speak(msg);
        }
        ...
        if (!full_text) {
          const msg = message || "No document text detected.";
          setOutput("📄 " + msg);
          speak(msg);
        } else {
          const msg = "Reading document.";
          setOutput("📄 " + full_text);
          speak(msg + " " + full_text);
        }
      }
